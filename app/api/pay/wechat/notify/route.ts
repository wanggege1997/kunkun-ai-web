...
import { getWechatNotifySecret, verifyPayloadSignature } from '@/lib/payment';
...
export async function POST(request: Request) {
  ...
  const secret = getWechatNotifySecret();
  if (!secret) {
    return NextResponse.json({ success: false, message: 'WECHAT_NOTIFY_SECRET 未配置' }, { status: 500 });
  }
